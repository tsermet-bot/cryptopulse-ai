...
def generate_pdf_report(coin_name, price, rsi, signal, galaxy_score):
    ...
    pdf.multi_cell(0, 8, "Disclaimer: This document is generated automatically by CryptoPulse AI Terminal for institutional information purposes only.")
...
if data and 'market_data' in data:
    ...
    # --- 20 INSTITUTIONAL TABS (PLACED AT THE TOP) ---
    ...
    # 20. EXPORT INSTITUTIONAL PDF REPORT
    with t_pdf:
        st.markdown("### 📄 Institutional Executive PDF Report Generator")
        st.caption("Generate a downloadable PDF audit report containing AI indicators, RSI, and market metrics.")
        if st.button("📥 Generate PDF Executive Report"):
            pdf_bytes = generate_pdf_report(selected_coin_name, price, current_rsi, signal_text, galaxy_score)
            st.download_button(
                label="💾 Download Report (PDF)",
                data=pdf_bytes,
                file_name=f"{selected_coin_name}_Institutional_Report.pdf",
